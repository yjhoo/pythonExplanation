...
# 1. 레이저는 여는 괄호와 닫는 괄호의 인접한 쌍 ‘( ) ’ 으로 표현된다. 또한, 모든 ‘( ) ’는 반     드시 레이저를 표현한다. 2. 쇠막대기의 왼쪽 끝은 여는 괄호 ‘ ( ’ 로, 오른쪽 끝은 닫힌 괄호 ‘) ’ 로 표현된다.
...
for i in range(len(N)):
    # '('값이 들어오면 cnt 추가하고 check를 True로 바꿔줌
    if N[i] == '(':
        cnt += 1
        check = True
    # ')'가 을어왔을 경우 cnt를 -1해주고 check가 True일 경우에는 레이저이므로 cnt 만큼 추가 / False일 경우는 막대기가 끝난 것이므로 +1 해줌
    else:
        cnt -= 1
        if check:
            answer += cnt
