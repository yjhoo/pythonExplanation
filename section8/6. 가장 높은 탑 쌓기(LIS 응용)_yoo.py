...
# ▣ 입력설명 입력 파일의 첫째 줄에는 입력될 벽돌의 수가 주어진다. 입력으로 주어지는 벽돌의 수는 최대 100개이다. 둘째 줄부터는 각 줄에 한 개의 벽돌에 관한 정보인 벽돌 밑면의 넓이, 벽돌의 높 이 그리고 무게가 차례대로 양의 정수로 주어진다. 각 벽돌은 입력되는 순서대로 1부터연속적 인 번호를 가진다.
# ▣ 출력설명 첫 번째 줄에 가장 높이 쌓을 수 있는 탑의 높이를 출력한다.
# ▣ 입력예제 1                                   5 25 3 4 4 4 6 9 2 3 16 2 5 1 5 2
# ▣ 출력예제 1 10
...
for i in range(N):
    m = 0
    # 기존 d에 입력됩 최대값과 비교 및 무게값이 클경우 m 초기화
    for j in range(i-1, -1, -1):
        if l[i][2] < l[j][2] and d[j] > m:
            m = d[j]
    # 가장 큰 m 값에 이번에 추가될 높이 더해서 d[i] 새로 구하고 이 값이 최대값일 경우 cnt 초기화
    d[i] = m + l[i][1]
    if cnt < d[i]:
        cnt = d[i]
print(cnt)
